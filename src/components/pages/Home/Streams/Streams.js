...
export const Streams = () => {
  ...
  const greyTheme =
    authContext.theme === "dark" ? { color: "#D3D3D3" } : { color: "#555555" };
    ...
    <Home>
      <div>
        {streamContext.loading ? (
          <LoadingStream />
        ) : Array.isArray(streamContext.stream) &&
          streamContext.stream.length ? (
                      ...
                      <Grid item className="mt-2" direction="row">
                        <Grid item>
                          <Typography variant="subtitle2" style={greyTheme}>
                            {st.date} &nbsp; {st.time}{" "}
                            <span className="center-dot">•</span> {st.type}
                          </Typography>
                        </Grid>
                      </Grid>
